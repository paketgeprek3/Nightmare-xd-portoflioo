(function() {
  ...
  async function loadSettings() {
    ...
    if (data && data.value) {
      ...
      // Load Links Sosmed dari database ke HTML icon
  ...
  // --- FUNGSI EDIT URL IKON SOSMED SAAT MODE ADMIN ---
  function handleLinkClick(e, label) {
    if (_ue) { // Jika sedang di mode edit/admin
      e.preventDefault(); // Jangan buka tab baru dulu
      ...
      if (newLink !== null) {
        ...
        saveAbout(); // Langsung simpan ke Database Supabase
      }
    } else {
      // Jika mode publik tapi link belum diisi, jangan buka apa-apa
